...
/* COMMENT: Guard against double-injection from concurrent onAdded + onUpdated.
 * Content scripts are wrapped by Chrome; this flag prevents re-initialization. */
const __PF_ALREADY_INITIALIZED = !!window.__OPM_PROMPT_MANAGER_INITIALIZED;
window.__OPM_PROMPT_MANAGER_INITIALIZED = true;
...
class PromptStorageManager {
  ...
  static async savePrompt(prompt) {
    const ps = await this._ps();
    const result = await ps.savePrompt(prompt);
    // Add initial version
    if (result && result.prompt && result.prompt.uuid && result.prompt.content) {
      try {
        await PromptStorageManager.addVersion(result.prompt.uuid, result.prompt.content, 'user_input');
      } catch {}
      // Auto-suggest metadata for first-time saves
      try {
        await PromptUI.autoSuggestMetadata(result.prompt.uuid, qs(`#${SELECTORS.PROMPT_LIST}`));
      } catch {}
    }
    return result;
  }
  ...
  static async addVersion(promptUuid, content, source = 'manual_edit', metadata = {}) {
    const ps = await this._ps();
    try { return await ps.addVersion(promptUuid, content, source, metadata); } catch { return null; }
  }

  static async getVersions(promptUuid) {
    const ps = await this._ps();
    try { return await ps.getVersions(promptUuid); } catch { return []; }
  }

  static async restoreVersion(promptUuid, versionIndex) {
    const ps = await this._ps();
    try { return await ps.restoreVersion(promptUuid, versionIndex); } catch { return null; }
  }
...
class PromptUIManager {
  ...
  static handleKeyboardNavigation(e, context = 'list') {
    ...
    const idx = context === 'search' ? PromptUIManager.selectedSearchIndex : items.indexOf(document.activeElement);
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      if (context === 'search') {
  ...
  static async showEditForm(prompt /*, index */) {
    ...
    const form = PromptUI.Views.createPromptForm({
      ...
      initialCategory: prompt.category || null,
      submitLabel: 'Save Changes',
      onSubmit: async ({ title, content, tags, category }) => {
        const ps = await PromptStorageManager._ps();
        const update = { title, content };
        if (Array.isArray(tags)) update.tags = tags;
        if (category) update.category = category;
        // Add version
        await PromptStorageManager.addVersion(prompt.uuid, content, 'manual_edit');
    ...
    // Add action buttons above the save button
    (async () => {
      const actionRow = createEl('div', { styles: { display: 'flex', gap: '6px', marginBottom: '8px', flexWrap: 'wrap' } });

      // Version History button
      const vhBtn = createEl('button', { innerHTML: '📋 Versions', className: 'opm-button opm-standard' });
      vhBtn.style.cssText = 'padding:4px 10px;font-size:12px;cursor:pointer;border-radius:4px;border:1px solid #ccc;background:var(--opm-bg,#fff);color:var(--opm-text,#333);';
      vhBtn.addEventListener('click', () => PromptUI.showVersionHistory(prompt.uuid));
      actionRow.appendChild(vhBtn);

      // Improve button
      const impBtn = createEl('button', { innerHTML: '✨ Improve', className: 'opm-button opm-standard' });
      impBtn.style.cssText = 'padding:4px 10px;font-size:12px;cursor:pointer;border-radius:4px;border:1px solid #ccc;background:var(--opm-bg,#fff);color:var(--opm-text,#333);';
      impBtn.addEventListener('click', async () => {
        await PromptUI.improvePrompt(prompt.uuid, qs(`#${SELECTORS.PROMPT_LIST}`));
      });
      actionRow.appendChild(impBtn);

      // Variants button
      const varBtn = createEl('button', { innerHTML: '🔄 Variants', className: 'opm-button opm-standard' });
      varBtn.style.cssText = 'padding:4px 10px;font-size:12px;cursor:pointer;border-radius:4px;border:1px solid #ccc;background:var(--opm-bg,#fff);color:var(--opm-text,#333);';
      varBtn.addEventListener('click', async () => {
        await PromptUI.generateVariants(prompt.uuid, qs(`#${SELECTORS.PROMPT_LIST}`));
      });
      actionRow.appendChild(varBtn);

      const saveBtn = form.querySelector('.opm-button');
      if (saveBtn && saveBtn.parentNode) saveBtn.parentNode.insertBefore(actionRow, saveBtn);
      ...
      if (saveBtn) {
        ...
        newBtn.addEventListener('click', async e => {
          ...
          const catSel = form.querySelectorAll('.opm-input-field')[2]; // Third input is category select
          const t = titleIn.value.trim(), c = contentArea.value.trim();
          if (!t || !c) { alert('Please fill in both title and content.'); return; }
          const cat = catSel && catSel.tagName === 'SELECT' && catSel.value !== 'Uncategorized' ? catSel.value : null;
          const ps = await PromptStorageManager._ps();
          await PromptStorageManager.addVersion(prompt.uuid, c, 'manual_edit');
          await ps.updatePrompt(prompt.uuid, { title: t, content: c, tags: tagInput.getTags(), category: cat });
          PanelRouter.mount(PanelView.EDIT);
        }, { once: true });
      }
      const listElAfter = qs(`#${SELECTORS.PROMPT_LIST}`);
  ...
  static async showVersionHistory(promptUuid) {
    try {
      const ps = await PromptStorageManager._ps();
      const versions = await ps.getVersions(promptUuid);
      if (!versions || versions.length === 0) { alert('No version history yet.'); return; }
      const overlay = createEl('div', { className: 'opm-modal-overlay' });
      ...
      modal.style.cssText = 'background:var(--opm-bg,#fff);border-radius:12px;padding:20px;max-width:600px;width:90%;max-height:80vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,0.3);';
      const header = createEl('div', { styles: { display: 'flex', justifyContent: 'space-between', marginBottom: '16px' } });
      header.appendChild(createEl('h3', { styles: { margin: 0, fontSize: '16px' } }, ['Version History']));
      const closeBtn = createEl('button', { innerHTML: '×', className: 'opm-button-close' });
      closeBtn.style.cssText = 'background:none;border:none;font-size:24px;cursor:pointer;color:var(--opm-text,#333);';
      closeBtn.addEventListener('click', () => overlay.remove());
      header.appendChild(closeBtn);
      modal.appendChild(header);
      for (let i = versions.length - 1; i >= 0; i--) {
        const v = versions[i];
        const row = createEl('div', { styles: { padding: '8px 0', borderBottom: '1px solid var(--opm-border,#e0e0e0)' } });
        row.appendChild(createEl('div', { innerHTML: `<b>v${i + 1}</b> <span style="color:#666;font-size:12px;">${v.source} · ${new Date(v.createdAt).toLocaleString()}</span>` }));
        const btns = createEl('div', { styles: { display: 'flex', gap: '4px', marginTop: '4px' } });
        const viewBtn = createEl('button', { innerHTML: 'View', className: 'opm-button opm-standard' });
        viewBtn.style.cssText = 'padding:4px 10px;font-size:12px;cursor:pointer;border-radius:4px;border:1px solid #ccc;';
        viewBtn.addEventListener('click', () => {
          alert(`Version ${i + 1} (${v.source}):\n\n${v.content}`);
        });
        btns.appendChild(viewBtn);
        if (i < versions.length - 1) {
          const restoreBtn = createEl('button', { innerHTML: 'Restore', className: 'opm-button opm-standard' });
          restoreBtn.style.cssText = 'padding:4px 10px;font-size:12px;cursor:pointer;border-radius:4px;border:1px solid #ccc;';
          restoreBtn.addEventListener('click', async () => {
            await ps.restoreVersion(promptUuid, i);
            overlay.remove();
            alert(`Restored as v${versions.length}`);
            await PanelRouter.mount(PanelView.LIST);
          });
    ...
    } catch (e) { console.error('Version history failed:', e); alert('Failed to load version history: ' + e.message); }
  }

  static async improvePrompt(promptUuid, listEl) {
    const { improvePrompt: fn } = await import(chrome.runtime.getURL('ai-operations.js'));
    return fn(promptUuid, listEl);
  }

  static async generateVariants(promptUuid, listEl) {
    const { generateVariants: fn } = await import(chrome.runtime.getURL('ai-operations.js'));
    return fn(promptUuid, listEl);
  }

  static async autoSuggestMetadata(promptUuid, listEl) {
    const { autoSuggestMetadata: fn } = await import(chrome.runtime.getURL('ai-operations.js'));
    return fn(promptUuid, listEl);
  }
...
const PromptMediator = (() => {
  ...
  /**
   * COMMENT: Check if we have permission for the current site's provider.
   * @returns {Promise<boolean>}
   */
  const checkCurrentSitePermission = async () => {
    try {
      ...
      for (const provider of data.llm_providers) {
        const regexPattern = provider.pattern
          .replace(/\\/g, '\\\\')
          .replace(/[.]/g, '\\.')
          .replace(/[*]/g, '.*');
        const urlRegex = new RegExp(`^${regexPattern}`);
        if (urlRegex.test(currentUrl)) {
          const hasPermission = await chrome.permissions.contains({ origins: [provider.pattern] });
          return hasPermission;
        }
      }
    } catch (e) {
      console.warn('[PromptForge] Failed to check site permission:', e);
    }
    return false;
  };

  /**
   * COMMENT: Copy prompt to clipboard and show notification.
   * @param {string} text
   */
  const copyToClipboardWithFeedback = async (text) => {
    try {
      ...
      console.error('[PromptForge] Failed to copy to clipboard:', e);
    }
  };

  /**
   * COMMENT: Main prompt selection handler reused across listeners.
   * Tries direct injection first, falls back to clipboard if input not found.
   * @param {Prompt} prompt
   */
  const handlePromptSelect = async (prompt) => {
    const listEl = qs(`#${SELECTORS.PROMPT_LIST}`);

    // Be resilient — if input box isn't ready yet, wait briefly before giving up
    let inputBox = await InputBoxHandler.getInputBox();
    if (!inputBox) {
      try {
        inputBox = await InputBoxHandler.waitForInputBox();
      } catch (_) {
        console.error('[PromptForge] Input box not found, falling back to clipboard.');
        await copyToClipboardWithFeedback(prompt.content);
        if (listEl) PromptUIManager.hidePromptList(listEl);
        return;
      }
    }
    ...
    // Prompt has #variables# → show variable input form
...
/* ===========================================================================
   External message handler — receives PROMPTFORGE_INSERT from full-page app
   =========================================================================== */
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  // COMMENT: Handle context menu prompt send — routes through variable processing
    ...
    (async () => {
      try {
        ...
        if (vars.length === 0) {
          ...
          if (inputBox) {
            await InputBoxHandler.insertPrompt(inputBox, content, null);
          } else {
        ...
        // Has variables — mount variable input form
        let inputBox = await InputBoxHandler.getInputBox();
        if (!inputBox) {
          inputBox = await InputBoxHandler.waitForInputBox();
        }
  ...
  if (message?.type !== 'PROMPTFORGE_INSERT') return false;

  (async () => {
    try {
      // Check if InputBoxHandler is available
      if (!window.InputBoxHandler) {
        try {
          await navigator.clipboard.writeText(message.text);
          sendResponse({ success: true, clipboard: true });
        } catch (e) {
          sendResponse({ success: false, error: 'Clipboard failed: ' + e.message });
        }
        return;
      }

      // Try to find the input box with retries
      let inputBox = null;
      for (let attempt = 0; attempt < 20; attempt++) {
        inputBox = await window.InputBoxHandler.getInputBox();
        if (inputBox) break;
      ...
      if (!inputBox) {
        ...
        try {
          ...
          sendResponse({ success: false, error: 'Could not find input field and clipboard failed: ' + e.message });
        }
        return;
      }

      await window.InputBoxHandler.insertPrompt(inputBox, message.text, null);
      sendResponse({ success: true });
    } catch (e) {
      console.error('[PromptForge] External insert failed:', e);
      sendResponse({ success: false, error: e.message });
    }
  })();
...
/* Initialize the extension — skip if already initialized on this page */
if (!__PF_ALREADY_INITIALIZED) {
